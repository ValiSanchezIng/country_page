...
export function WeeklyCalendar({ userLevel, userId, userType, onSlotClick, userBookings = [], allWeekBookings = [], className, claseCupoMax, claseId, onWeekChange }) {
  ...
  const generateTimeSlots = (day, realDate) => {
    ...
    timeSlots.forEach((time) => {
      ...
      // El backend (calcularInstructorasDisponibles) ya descuenta a las instructoras en
      // descanso y a las que ya están dando OTRA clase (no iniciación/ponyclub) a esta misma
      // hora, por lo que este número es la disponibilidad real y se usa directamente.
